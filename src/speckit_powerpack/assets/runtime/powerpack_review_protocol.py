...
def validate_review(review: dict[str, Any], previous: dict[str, Any] | None = None) -> list[str]:
    ...
    if previous is not None:
        expected = {str(item.get("id")) for item in _list(previous.get("findings")) if isinstance(item, dict) and item.get("id")}
...
def cmd_validate(args: argparse.Namespace) -> int:
    ...
    payload = {"valid": not errors, "schema_version": SCHEMA_VERSION, "verdict": review.get("verdict"), "errors": errors}
